...
def signup():
    if request.method == "POST":
        ...
        try:
            ...
            db.session.rollback()
            flash("Username or email already exixts! Try with new credentials")
...
@app.route('/admin/<int:curr_login_id>/dashboard',methods=["GET","POST"])
    ...
    if request.method == 'GET':
        if 'user_id' in session and session['user_id'] == curr_login_id:
            ...
            if user_cart:  
                ...
                data = {'curr_login_id': curr_login_id, 'cart': {}}
